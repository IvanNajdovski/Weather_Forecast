...
app.get("/", function(req,response){
    ...
    var url = `https://farm${res.data.photos.photo[number].farm}.staticflickr.com/${res.data.photos.photo[number].server}/${res.data.photos.photo[number].id}_${res.data.photos.photo[number].secret}.jpg`;
        response.render("home", {temp: wedRes.currently.temperature, name: location , photo: url})
...
app.post("/", function(req,response){
    ...
    var temperature = response.data.currently.temperature;
    var apparentTemperature = response.data.currently.apparentTemperature;
    var picLoc = `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=c11bf5d4dd61901c8e1fb260bf4a1ad0&content_type=screenshot&text=site&lat=${lat}&lon=${lng}&format=json&nojsoncallback=1`;
    return axios.get(picLoc);
}).then((res)=>{
        var number = Math.round(Math.random()*10);
    var url = `https://farm${res.data.photos.photo[number].farm}.staticflickr.com/${res.data.photos.photo[number].server}/${res.data.photos.photo[number].id}_${res.data.photos.photo[number].secret}.jpg`;
